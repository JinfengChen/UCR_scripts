...
#SBATCH --output=buscov3_array.%A_%a.stdout
#SBATCH -p intel
#SBATCH --workdir=./
...
if [ ! -e "run_$output" ]; then
   python ~/BigData/00.RD/Assembly/Pacbio/install/BUSCO/busco/scripts/run_BUSCO.py --in $genome --cpu $CPU --out $output --lineage_path $lineage --mode geno --tmp_path $temp
fi
end=`date +%s`
runtime=$((end-start))
