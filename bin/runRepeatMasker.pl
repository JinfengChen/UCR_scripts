...
my $outdir=".";
my $script="/rhome/cjinfeng/BigData/software/bin";
my $repeat2gff="$script/repeat_to_gff.pl";
my $fastadeal="$script/fastaDeal.pl";
my $repeatmasker="/opt/linux/centos/7.x/x86_64/pkgs/RepeatMasker/4-0-5/RepeatMasker";
my $statTE="$script/stat_TE.pl";
my $qsub="$script/qsub-slurm.pl";
...
foreach (@subfiles){
    ...
    print OUT "$repeatmasker -pa 24 -lib $rmlib -q -nolow -no_is -norna $_ > $_.log 2> $_.log2\n";
}
close OUT;

## run shell by qsub-sge.pl
`perl $qsub --maxjob 60 --lines 1 --interval 120 --task 24 --mem 40G --time 20:00:00 $repeatshell`;
`rm $outdir/$filename.cut/*.ori.out`;
