...
def convert_fasta(infile):
    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            line = line.rstrip()
            if line.startswith(r'BEGIN GENOTYPES'):
                flag = 1
            elif line.startswith(r'END GENOTYPES'):
                flag = 0
            elif flag == 1:
                if line.startswith(r'#'):
                    strain_n += 1
                    strain = re.sub(r'# ', r'', line)
                else:
                    hap = re.sub(r' ', r'', line)
                    loci_n = len(hap)
                    data[strain].append(hap)
            else:
                continue
    ofile = open('%s.fasta' %(infile), 'w')
    for strain in sorted(data.keys()):
        for i in [0]:
            print >> ofile, '>%s\n%s' %(strain, data[strain][i])
